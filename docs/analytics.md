...
| `recipient_send_cta_view` / `recipient_send_cta_click` | 全ファイルのダウンロード完了から1.5秒後にモーダル表示される「Anzdropでファイルを送る」導線。利用者が「次から表示しない」を選ぶと、このブラウザの`localStorage`だけへ設定を保存し、以後は表示・計測しない |
