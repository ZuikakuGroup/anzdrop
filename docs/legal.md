...
- 事業者情報と3ページ共通の最終改定日は [`lib/legal/constants.ts`](../lib/legal/constants.ts) の `OPERATOR` / `OPERATOR_ENTITY_LABEL` / `LEGAL_LAST_UPDATED` に集約している。3ページとも、これらをハードコードせずここから参照する。
  - 本サービスは任意団体「瑞鶴グループ」として運営している。特定商取引法に基づく表記の「販売事業者」は団体名(「任意団体 瑞鶴グループ」)で表示し、運営統括責任者に運営者個人の氏名(`OPERATOR.representative`)を記載する。利用規約・プライバシーポリシー本文で「当方」を定義する主体表記は `OPERATOR_ENTITY_LABEL`(「任意団体「瑞鶴グループ」(運営者: 相澤遼)」)で、こちらは運営者個人の氏名を含む。
