...
- ロック中でも本人を通す緩和により、失敗回数ロックのレートリミット効果はロック期間中に限れば弱まる(ロック期間内に最大 20 回まで本物のパスワード照合が走る)。攻撃者が正規ユーザーの再ログインを妨げ続けるにはロック期間ごとに 20 個の Turnstile トークンを消費する必要があり、その間も正規ユーザーはロック明けに必ず入り直せる。
