...
アカウントIDはランダム発行ではなく本人が自由に選べるため、IDの予測不可能性には頼れない(以前はアカウントIDの長い・ランダムな文字列自体が、パスワードと並ぶ総当たり対策の一部だった)。代わりに、`POST /api/account/login`(`app/api/account/login/route.ts`)はアカウントごとにログイン失敗回数を記録し、**5回連続で失敗すると5分間そのアカウントIDでのログインを一時的に制限する**。成功時・またはリカバリーによるパスワード再設定時に失敗回数とロックはリセットされる。この情報(`accounts.failed_login_attempts`・`accounts.locked_until`、migration 0012)はログインのたびに参照・更新される、アカウントごとのカウンタのみで、個人を特定する情報ではない。

**ロック中でも、正しいパスワード(と Turnstile)を提示できる本人はログインを通す**(GitHub issue #66)。アカウントIDが公開されうる文字列である以上、第三者が対象IDへ誤ったパスワードを連打するだけで正規ユーザーを継続的に締め出せる(標的型ロックアウト嫌がらせ)ため。ただし**ロック期間内の試行が一定回数(`LOGIN_LOCKOUT_THRESHOLD × 4 = 20`)を超えたら、以降はパスワード照合をダミーにして 403 にする**(自動化された総当たりをロック期間内で頭打ちにする)。ロック中の誤ったパスワードは通常どおり 403 で、**ロック期間は延長しない**(延長すると嫌がらせでロックが伸び続けてしまう)。ロック期間が過ぎれば、積み上がった失敗カウンタごとクリアして通常のフローに戻る。総当たりに対する主たる防御は **ログインごとに必須の Turnstile**(自動化された大量試行にはトークン取得コストがかかる)と **Argon2id**(1回の照合が数十ms)であり、失敗回数ロックはあくまで補助という位置づけ。

**既知の制約:**

- メールアドレスを収集しないため、標的型ロックアウト嫌がらせに対する回復手段はリカバリーコード(`/api/account/recover`。ロック状態でも通る)と「正しいパスワードならロック中でも通す」緩和のみ。
- ロック中でも本人を通す緩和により、失敗回数ロックのレートリミット効果はロック期間中に限れば弱まる(ロック期間内に最大 20 回まで本物のパスワード照合が走る)。攻撃者が正規ユーザーの再ログインを妨げ続けるには、ロック期間ごとに 20 個の Turnstile トークンを消費して照合枠を先に使い切る(以降その期間は本人でも 403)か、ロック明けに 5 回誤入力して再施錠する必要がある。いずれもロック期間(5分)ごとに継続的なコストがかかり、その隙間(照合枠が残っている間、または再施錠前)に本人が正しいパスワードで入り直せる。完全に締め出し続けることはできないが、嫌がらせを完全に無害化するわけではない(本命の対策は下記の IP 単位カウント)。
- IP 単位・Turnstile 単位でのカウント(サーバー側に新しい情報を持たない範囲では難しい)は導入していない。より強いレートリミットが必要になった場合の本命の対策。
...
- 対象アカウントの `stripe_subscription_id` が **Stripe 上でいま実際に管理対象として生きている契約**(`active` / `trialing` / `past_due`。`isManageableSubscriptionStatus()`)を指している場合は画面上で警告する。付与・取り消しをしても次回の `sync` / Webhook が Stripe 上の実際の契約内容へ上書きしうるため。カード契約自体を止めるには Stripe 側で解約する。`GET` / `POST` / `DELETE` いずれも `stripe.subscriptions.retrieve()` で現在のステータスを確認しており、ポインタの有無だけでは判定しない(決済フォームを開いて離脱しただけの `incomplete` / `incomplete_expired` のゴミポインタでは警告を出さない)。`retrieve` に失敗した場合(404・レート制限・障害)は `sync` の 404 方針と同じく「実態を確認できなかった」とみなして保守的に警告を残す(`true`)。この確認は表示専用で、`accounts` は書き換えない(reconcile は `sync` に委ねる)。
