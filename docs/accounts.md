...
**ロック中でも、正しいパスワード(と Turnstile)を提示できる本人はログインを通す**(GitHub issue #66)。アカウントIDが公開されうる文字列である以上、第三者が対象IDへ誤ったパスワードを連打するだけで正規ユーザーを継続的に締め出せる(標的型ロックアウト嫌がらせ)ため。ロック中の誤ったパスワードは通常どおり 403 で、ロックと失敗カウンタはそのまま残り、時間経過で自然に解除される。総当たりに対する主たる防御は **ログインごとに必須の Turnstile**(自動化された大量試行にはトークン取得コストがかかる)と **Argon2id**(1回の照合が数十ms)であり、失敗回数ロックはあくまで補助という位置づけ。

**既知の制約:** メールアドレスを収集しないため、標的型ロックアウト嫌がらせに対する回復手段はリカバリーコード(`/api/account/recover`。ロック状態でも通る)と「正しいパスワードならロック中でも通す」緩和のみ。IP 単位・Turnstile 単位でのカウント(サーバー側に新しい情報を持たない範囲では難しい)は導入していない。
