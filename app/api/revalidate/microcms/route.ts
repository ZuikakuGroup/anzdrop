...
export async function POST(request: Request): Promise<Response> { const { env: baseEnv } = getCloudflareContext(); const env = baseEnv as CloudflareEnv & { MICROCMS_WEBHOOK_SECRET?: string }; const received = hexToBytes(request.headers.get("x-microcms-signature") ?? ""); if (!env.MICROCMS_WEBHOOK_SECRET || !received) return Response.json({ error: "Unauthorized" }, { status: 401 }); const rawBody = await readBodyWithinLimit(request, 64 * 1024); if (!rawBody) return Response.json({ error: "Payload Too Large" }, { status: 413 }); const body = new TextDecoder().decode(rawBody); if (!constantTimeEqual(await signature(body, env.MICROCMS_WEBHOOK_SECRET), received)) return Response.json({ error: "Unauthorized" }, { status: 401 }); let payload: unknown; try { payload = JSON.parse(body); } catch { return Response.json({ error: "Invalid JSON" }, { status: 400 }); } if (!payload || typeof payload !== "object" || !("api" in payload) || typeof payload.api !== "string" || !TARGET_APIS.has(payload.api)) return Response.json({ error: "Unknown API" }, { status: 400 }); return Response.json({ accepted: true }); }
